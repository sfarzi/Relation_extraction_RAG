...
for id in all.keys():
    ...
    not_found = True
    # print(testrels)
    for restuple in resrels:
        result_chemical = restuple[0].lower()
        result_disease = restuple[1].lower()
        for testtuple in testrels:
            if result_chemical in testtuple[0].lower():
                if result_disease in testtuple[1].lower():
                    # print(f"{restuple}\n{testtuple}\n\n")
                    testrels.remove(testtuple)
                    tp += 1
                    not_found = False
                    break
